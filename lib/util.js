	...
	if(Meteor.isClient){
		if(Session.get('sPartieId')){
			if(Session.get('sPartieId') != partieId){
				...
				Meteor.call("launchEvenement", Session.get('sPartieId'));
			}
		}
	}
}
...
interprete = function(condition, nG, strict){
	...
	do {
		...
		if (condition[n] == ")"){
			...
			//if(resultRecc.hasExpression){
				...
				//console.log("ligne 227" + resultRecc.mongo);
			//} else {
	...
	} else if ((nG > 0) && (termes.length > 1) && (result.hasExpression)) {
	...
	if((expressions.length > 0) || (termes.length > 0)) result.mongo = result.mongo.substring(0, (result.mongo.length - 2));
	...
	if ((nG == 0) && (termes.length > 1) && (!result.hasExpression)){
		//*-*-* result.mongo += '}}';
	} else if ((nG > 0) && (termes.length > 1) && (result.hasExpression)) {
		//*-* result.mongo += '}}';
		if(op != ""){
	...
	if((result.mongo.length > 1) && (result.mongo[result.mongo.length - 2] == ",")) result.mongo = result.mongo.substring(0, result.mongo.length-2);
