...
Meteor.methods({
		...
		var partie = {userId: userId, nom: TAPi18n.__("partie.nouvelle"), dateDebut: new Date(), dateModif: new Date(), deck:{cartes:[], energieMax:0, budgetMax:0, masseMax:0, volumeMax:0, energie:0, budget:0, masse:0, volume:0, science:0}, budgetLanceur: 0.5, eventIds:[], scenarioId: scenario._id, score: 0, logs:[], peers:[], experts:[], chat:[], finie: false, active: true};	
  		try{
			if(scenario.initialisation.cubesat){
				...
				partie.deck.masseMax  = partie.deck.masse;
				partie.deck.volumeMax = partie.deck.volume;
			} else {
				partie.deck.masse  =  0;
	...
	*/
	updatePercentLanceur: function(partie, scenario, percent, dateModif){
		var user = Meteor.user();
  		if (!(user)) { return throwError("privilege_user", TAPi18n.__("error.privilege_anonyme")); }
		try {
	...
	*/
	...
	addCarteToDeck: function(partieId, scenario, carte, dateModif){
		...
		try{
			...
			console.log(partie.deck.budget + " ? " + getValeurDeRegle(partie._id, carte.valEur));
			...
			if(affectedRow) {
				...
				if(Meteor.isClient) {
					console.log("CLIENT - recorded OK" + affectedRow);
				} else {
					console.log("SERVEUR - recorded OK" + affectedRow);
				}	
			} 
		} catch(e){
	...
	*/
	removeCarteFromDeck: function(partieId, scenario, carte, dateModif){
		...
		var affectedRow = false;
		try{
			console.log("REMOVE "+carte._id+ " FROM "+partie._id);
			affectedRow = Parties.update({_id: partie._id}, 
				{$push: { logs:  log },
	...
	*/
	callExpert: function(partie, scenario, dateModif){
		var user = Meteor.user();
  		if (!(user)) { return throwError("privilege_user", TAPi18n.__("error.privilege_anonyme")); }
  		try {
 			check(partie, Match.ObjectIncluding({ _id: Meteor.Collection.ObjectID, nom: String, userId: Meteor.Collection.ObjectID }));
	...
	*/
	updateConstantes: function(partie, scenario, dateModif){
		...
		console.log("UPD1 :" + partie.deck.budget);
		//parcours des cartes du deck
		var carte  = null;
		var valeur = 0;
		for(var c=0; c < partie.deck.cartes.length; c++){
			console.log("UPDc :"+ partie.deck.cartes[c]._carteId);
			carte = Cartes.findOne({_id: partie.deck.cartes[c]._carteId});
			console.log("UPDcarte :"+ carte._id);
			...
			if(partie.deck.cartes[c].active){//carte désactivée dans la partie, elle n'a plus d'impact sur l'énergie et la science mais bien sur le budget, la masse et le volume.
				console.log("valNrg" +  carte.valNrg);			
				...
				console.log("valSci" +  carte.valSci);
			...
			if(contains(carte.tags, "lanceur")){//% du lanceur utilisé
				console.log("UPD : L(" + partie.budgetLanceur +") V(" + valeur +")" );
				partie.deck.budget -= Math.floor(partie.budgetLanceur * valeur);
			} else {
				partie.deck.budget -= valeur;
			}
			console.log("UPD2 :" + partie.deck.budget);
		...
		try {
			...
			console.log("MAJ CONSTANTES : db.parties.update({_id: "+partie._id+"},{$set:{'deck.budget': "+partie.deck.budget+"}})");
