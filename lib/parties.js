				...
				try {
					...
					if(affectedRow){
						...
						for(var i=0; i < evenement.targetCarteIds.length; i++){
							partie.deck.cartes.forEach(c){ if(c._carteId === evenement.targetCarteIds[i]) c.active = false; };
						}
						for(var i=0; i < evenement.targetCarteTags.length; i++){
							partie.deck.cartes.forEach(c){ if(contains(c.tags, evenement.targetCarteTags[i])) c.active = false; };
						}
