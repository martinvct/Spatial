...
Meteor.methods({
	...
	*/
	newPartie: function(_scenarioId){
		...
		var partie = {userId: userId, nom: TAPi18n.__("partie.nouvelle"), dateDebut: new Date(), dateModif: new Date(), deck:{cartes:[], energieMax:0, budgetMax:0, masseMax:0, volumeMax:0, energie:0, budget:0, masse:0, volume:0, science:0}, budgetLanceur: 0.5, eventIds:[], scenarioId: scenario._id, score: 0, logs:[], peers:[], experts:[], chat:[], finie: false, active: true};	
  		try{
			partie.deck.masse     =  0;
	...
	addCarteToDeck: function(partieId, scenario, carte, dateModif){
		...
		if(affectedRow){
			...
			if(user.userId != partie.userId){
				Meteor.call("trowNotification", "DECK-ADD", partie.userId, {_partieId: partie._id, nom: partie.nom, _carteId: carte._id, intitule: carte.intitule.fr, userId: user._id})
			}
			partie.peers.forEach(function(p){ if((user.userId != p.userId) && (p.dateDebut + p.dureeMax >= now)) Meteor.call("trowNotification", "DECK-ADD", p.userId, {_partieId: partie._id, nom: partie.nom, _carteId: carte._id, intitule: carte.intitule.fr, userId: user._id}) });
	...
	* @result : none
	*/
	addMessage:function(partie, scenario, message, dateModif){
		var user = Meteor.user();
  		if (!(user)) { return throwError("privilege_user", TAPi18n.__("error.privilege_anonyme")); }
  		try {
	...
	*/
	...
	validationDeck: function(partie, scenario, niveauDetails, isLancement) {
		...
		console.log("validationDECK 1 ("+partie._id+")");
		if(scenario.initialisation.cubesat){
			console.log("validationDECK 2");
			if(scenario.initialisation.cubesatOptions.nU == "1"){
				console.log("validationDECK 3");
				for(var t=0; t < ListTagsObligatoires.cubesat1U.length; t++){
					console.log("validationDECK 3+1U:"+ListTagsObligatoires.cubesat1U[t].tag);
					...
					// db.parties.aggregate([{$match: {_id: "kCW9MS4YXvypEr3mG"}}, {$unwind: "$deck.cartes"}, {$unwind: "$deck.cartes.tags"}, {$match: {"deck.cartes.tags": "lanceur1U", "deck.cartes.active": true }}, {$group: {"_id":"$_id", "nbr":{$sum: 1}}}]);
					if(!objResult || objResult.nbr == 0){
						console.log("validationDECK 3+KO");
						switch(niveauDetails){
							case 0:
								erreurs = [true];
								break;
							case 1:
								erreurs[ListTagsObligatoires.cubesat1U[t].categorie] =  ListTagsObligatoires.cubesat1U[t].categorie;
								break;
							case 2: 
								erreurs.push({categorie: ListTagsObligatoires.cubesat1U[t].c, cartes: Cartes.find({tags: ListTagsObligatoires.cubesat1U[t]},{_id: 1, carteId: 1, intitule: 1})});
								break;		
						}
					}
					...
					if(!objResult || objResult.nbr == 0){
						switch(niveauDetails){
							case 0:
								...
								break;
							case 1:
								erreurs[ListTagsObligatoires.cubesat3U[t].categorie] =  ListTagsObligatoires.cubesat3U[t].categorie;
								break;
							case 2: 
								erreurs.push({categorie: ListTagsObligatoires.cubesat3U[t].c, cartes: Cartes.find({tags: ListTagsObligatoires.cubesat3U[t]},{_id: 1, carteId: 1, intitule: 1})});
								break;		
						}
					}
				}
				...
				if(!objResult || objResult.nbr == 0){
					switch(niveauDetails){
						case 0:
							...
							break;
						case 1:
							erreurs[ListTagsObligatoires.cubesat[t].categorie] =  ListTagsObligatoires.cubesat[t].categorie;
							break;
						case 2: 
							erreurs.push({categorie: ListTagsObligatoires.cubesat[t].c, cartes: Cartes.find({tags: ListTagsObligatoires.cubesat[t]},{_id: 1, carteId: 1, intitule: 1})});
							break;		
					}
				}
			}
		...
		if(!scenario.initialisation.cubesat){
				...
				if(!objResult || objResult.nbr == 0){
					switch(niveauDetails){
						case 0:
							...
							break;
						case 1:
							erreurs[ListTagsObligatoires.global[t].categorie] = ListTagsObligatoires.global[t].categorie;
							break;
						case 2: 
							erreurs.push({categorie: ListTagsObligatoires.global[t].c, cartes: Cartes.find({tags: ListTagsObligatoires.global[t]},{_id: 1, carteId: 1, intitule: 1})});
							break;		
					}
				}
			}
			...
			if(scenario.initialisation.objectif === "espace"){
				for(var t=0; t < ListTagsObligatoires.espace.length; t++){
					objResult = Parties.aggregate([{$match: {_id: partie._id}}, {$unwind: "$deck.cartes"}, {$unwind: "$deck.cartes.tags"}, {$match: {"deck.cartes.tags": ListTagsObligatoires.espace[t], "deck.cartes.active": true }}, {$group: {"_id":"$_id", "nbr":{$sum: 1}}}]);
					if(!objResult || objResult.nbr == 0){
						switch(niveauDetails){
							case 0:
								erreurs = [true];
								break;
							case 1:
								erreurs[ListTagsObligatoires.espace[t].categorie] =  ListTagsObligatoires.espace[t].categorie;
								break;
							case 2: 
								erreurs.push({categorie: ListTagsObligatoires.espace[t], cartes: Cartes.find({tags: ListTagsObligatoires.espace[t]},{_id: 1, carteId: 1, intitule: 1})});
								break;		
						}
					}
				}
			...
			if(scenario.initialisation.objectif === "planete"){
				if(partie.planete.distance > 5){
					for(var t=0; t < ListTagsObligatoires.distance[partie.planete.distance.toString()].length; t++){
						objResult = Parties.aggregate([{$match: {_id: partie._id}}, {$unwind: "$deck.cartes"}, {$unwind: "$deck.cartes.tags"}, {$match: {"deck.cartes.tags": ListTagsObligatoires.distance[partie.planete.distance.toString()][t], "deck.cartes.active": true }}, {$group: {"_id":"$_id", "nbr":{$sum: 1}}}]);
						if(!objResult || objResult.nbr == 0){
							switch(niveauDetails){
								case 0:
									erreurs = [true];
									break;
								case 1:
									erreurs[ListTagsObligatoires.distance[partie.planete.distance.toString()][t].categorie] =  ListTagsObligatoires.distance[partie.planete.distance.toString()][t].categorie;
									break;
								case 2: 
									erreurs.push({categorie: ListTagsObligatoires.distance[partie.planete.distance.toString()][t], cartes: Cartes.find({tags: ListTagsObligatoires.distance[partie.planete.distance.toString()][t]},{_id: 1, carteId: 1, intitule: 1})});
									break;		
							}
						}
					}
					...
					for(var t=0; t < ListTagsObligatoires.distance["10"].length; t++){
						objResult = Parties.aggregate([{$match: {_id: partie._id}}, {$unwind: "$deck.cartes"}, {$unwind: "$deck.cartes.tags"}, {$match: {"deck.cartes.tags": ListTagsObligatoires.distance["10"][t], "deck.cartes.active": true }}, {$group: {"_id":"$_id", "nbr":{$sum: 1}}}]);
						if(!objResult || objResult.nbr == 0){
							switch(niveauDetails){
								case 0:
									erreurs = [true];
									break;
								case 1:
									erreurs[ListTagsObligatoires.distance["10"][t].categorie] = ListTagsObligatoires.distance["10"][t].categorie;
									break;
								case 2: 
									erreurs.push({categorie: ListTagsObligatoires.distance["10"][t], cartes: Cartes.find({tags: ListTagsObligatoires.distance["10"][t]},{_id: 1, carteId: 1, intitule: 1})});
									break;		
							}
						}
					}
		...
		console.log("validationDECK 4");
		//if faut vérifier les constantes d'énergie, masse,...poids
		if(niveauDetails == 1) erreurs["_"] = [];
 		...
 		if(partie.deck.science == 0){
 			switch(niveauDetails){
				...
				case 1:
					...
					break;
				case 2: 
					erreurs.push({constante: "science", valeur: partie.deck.science});
					break;		
			}
 		}
 		console.log("validationDECK 5");
		...
		for(var d=0; d < partie.deck.cartes.length; d++){
			...
			console.log("validationDECK 5+"+carteDeck.carteId);
  			if(carteDeck.active) {
  				...
  				if(carte.regles.necessite.length > 0){
  					for(var r=0; r < carte.regles.necessite.length; r++){
  						console.log("validationDECK 5+"+carte.regles.necessite[r]);
  						...
  						console.log("validationDECK 5.1+");
  						console.log(objResult);

  						if(!objResult || objResult.length == 0 || objResult.nbr == 0){
  							console.log("validationDECK 5+: ERREUR"+carte.categorie+" nd:"+niveauDetails);
							switch(niveauDetails){
								case 0:
									erreurs = [true];
									break;
								case 1:
									erreurs[carte.categorie] = carte.categorie;
									break;
								case 2: 
