...
Router.configure({
  ...
  notFoundTemplate: 'NotFound'
	...
	Router.onBeforeAction(function(){
		...
		//	this.render('Home');
		//}
	}, { only: ['Config', 'ConfigScenarios', 'ConfigUtilisateurs', 'ConfigBatchUtilisateurs', 'EditScenario', 'ConfigCartes', 'ConfigCartesCategorie', 'EditCarte'] });
...
Router.route('/cartes', { name: 'ConfigCartes', template:'ConfigCartes', subscriptions: function(){ return Meteor.subscribe('getCartes', false); }, action: function(){ if(this.ready()) { this.render(); } else { this.render('Loading'); } } } );
Router.route('/cartes/categorie/:_categorieId/:_cubesat', { name: 'ConfigCartesCategorie', template:'ConfigCartesCategorie', subscriptions: function(){ return Meteor.subscribe('getCartes', false); }, data: function() { d = { categorie: this.params._categorieId, cubesat: this.params._cubesat }; return d; }, action: function(){ if(this.ready()) { this.render(); } else { this.render('Loading'); } } } );
Router.route('/carte/:carteId', { name: 'EditCarte', template:'EditCarte', subscriptions: function(){ return [Meteor.subscribe('getCartes', false), Meteor.subscribe('getTags'), Meteor.subscribe('getMyProfile')]; }, data: function() { d = { carte: Cartes.findOne({carteId: this.params.carteId}) }; return d; }, action: function(){ if(this.ready()) { this.render(); } else { this.render('Loading'); } } } );
//Router.route('/carte/new/:_categorieId/:_cubesat', { name: 'NouvelleCarte', template:'EditCarte', subscriptions: function(){ return Meteor.subscribe('getCartes', false); }, data: function() { d = { categorie: this.params._categorieId, cubesat: this.params._cubesat }; return d; } } );