	...
	*/
	...
	addCarteToDeck: function(partieId, scenario, carte, dateModif){
		...
		if(affectedRow){
			...
			partie.peers.forEach(function(p){ if((user.userId != p.userId) && (p.dateDebut + p.dureeMax >= now)) Meteor.call("throwNotification", "DECK-ADD", p.userId, {partieId: partie._id, scenarioId: partie.scenarioId, nom: partie.nom, carteId: carte.carteId, intitule: carte.intitule.fr, user: { userId: user._id, username: user.profile.username, firstname: user.profile.firstname, lastname: user.profile.lastname, avatar: user.profile.avatar }}) });
