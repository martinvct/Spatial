...
Meteor.methods({
	...
	*/
	setPlaneteToPartie: function(partie, scenario, planete){
		try{
			...
			check(planete, Match.ObjectIncluding({_id: String, nom: String, distance: Number, atmosphere: Boolean}));
		} catch(e){
	...
	*/
	...
	addCarteToDeck: function(partieId, scenario, carte, dateModif){
		...
		var affectedRow = false;
		try{
			if(partie.deck.budget < getValeurDeRegle(partie._id, carte.valEur)) { console.log("PAS ASSEZ"); return throwAlert("error","budget_insuffisant", TAPi18n.__("error.budget.insuffisant")); }
  		...
  		try {
  			...
  			for(var c=0; c < partie.peers.length; c++){ 
  				if((partie.peers[c].userId == peer._id) && ((scenario.collaboration.tpsMaxParAppel == 0) || (partie.peers[c].dateDebut.getTime() + scenario.collaboration.tpsMaxParAppel < now.getTime()))) {
  					...
  					return throwAlert("error","PEER-STILLCALL", TAPi18n.__("notification.peer-stillcall", { username: partie.peers[c].username, firstname: partie.peers[c].firstname, lastname: partie.peers[c].lastname, dateAppel: moment(partie.peers[c].dateAppel).format("DD/MM/YYYY HH:mm:ss") }));
  				}  
  			} 
  			if(partie.deck.budget - scenario.collaboration.malusBudget  >= 0) {
  				...
  				throwAlert("message","PEER-CALL", TAPi18n.__("notification.peer-call", { username: user.profile.username, firstname: user.profile.firstname, lastname: user.profile.lastname }));
  			} else {
