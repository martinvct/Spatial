...
Meteor.methods({
	...
	*/
	addCarteToDeck: function(partie, carte){
		...
		if(carte.active == 0) { throw new Meteor.Error(TAPi18n.__("error.carte_inactive")); }
		var now = new date();
		...
		if(affectedRow){
			...
			//>TODO ajout cartid dans deck.carteids ?
	...
	*/
	removeCarteFromDeck: function(partie, carte){
		...
		if(affectedRow){
			partie.dateModif = now;
			partie.logs.push(log);
			//TODO retirer carteId de deck.carteids ?
	...
	*/
	addMessage:function(partie, scenario, message){
		var user = Meteor.user();
  		if (!(user)) { throw new Meteor.Error(TAPi18n.__("error.privilege_anonyme")); }
  		check(partie, {userId : Meteor.Collection.ObjectID, chat: Match.Optional([Object]), peers: Match.Optional([{userId: Meteor.Collection.ObjectID, dateDebut: Match.OneOf(Number, Date), dureeMax: Number}]), finie: Boolean, active: Boolean});
  		check(scenario, {collaboration: {tpsMaxParAppel: Number}});
	...
	*/
	delMessage:function(partie, scenario, message){
		var user = Meteor.user();
  		...
  		check(partie, {userId : Meteor.Collection.ObjectID, chat: Match.Optional([Object]), peers: Match.Optional([{userId: Meteor.Collection.ObjectID, dateDebut: Match.OneOf(Number, Date), dureeMax: Number}]), finie: Boolean, active: Boolean});
  		check(scenario, {collaboration: {tpsMaxParAppel: Number}});
  		check(message, {auteurId: Meteor.Collection.ObjectID, dateTime: Date});
  		if(user._id != message.auteurId) { throw new Meteor.Error(TAPi18n.__("error.privilege_user")); }
  		if((partie.finie == 1) || (partie.active == 0)) { throw new Meteor.Error(TAPi18n.__("error.partie_close")); }
	...
	*/
	addEvenementToDeck: function(partie, evenement){
		var user = Meteor.user();
		check(partie, {userId : Meteor.Collection.ObjectID, chat: Match.Optional([Object]), eventIds: Match.Optional([Meteor.Collection.ObjectID]), peers: Match.Optional([{userId: Meteor.Collection.ObjectID, dateDebut: Match.OneOf(Number, Date), dureeMax: Number}]), finie: Boolean, active: Boolean});
		check(evenement, {eventId: Meteor.Collection.ObjectID, intitule: String, deltaTps: Number, deltaEur: Number, deltaNrg: Number, deltaPds: Number, deltaVol: Number, deltaSci: Number, active: Boolean});
		if((partie.finie == 1) || (partie.active == 0)) { throw new Meteor.Error(TAPi18n.__("error.partie_close")); }
		if(evenement.active == 0) { throw new Meteor.Error(TAPi18n.__("error.evenement_inactif")); }
		var affectedRow = false;
		try {
