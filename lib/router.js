...
Router.route('/partie/:_scenarioId/:_id', { name:'Partie', template:'Partie', subscriptions: function(){ return [ Meteor.subscribe('illustrations'), Meteor.subscribe('getScenario', this.params._scenarioId, false), Meteor.subscribe('getPartie', this.params._id), Meteor.subscribe('getCartes', true), Meteor.subscribe('getUsers')]; }, data: function() { d = { scenarioId: this.params._scenarioId, partieId: this.params._id }; return d; }, action: function(){ if(this.ready()) { this.render(); } else { this.render('Loading'); } } }); 
