...
Meteor.methods({
	...
	*/
	...
	addCarteToDeck: function(partieId, scenario, carte, dateModif){
		...
		var affectedRow = false;
		try{
			if(partie.deck.budget < getValeurDeRegle(partie._id, carte.valEur)) { console.log("PAS ASSEZ"); return throwAlert("error","budget_insuffisant", TAPi18n.__("error.budget_insuffisant")); }
		  			...
		  			if(carteDeck.active) {
		  				...
		  				if(carte.regles.necessite.length > 0){
		  					for(var r=0; r < carte.regles.necessite.length; r++){
		  						console.log(carte.regles.necessite[r]);
			...
			if(scenario.initialisation.objectif === "planete"){
				...
				if(hasAtterrisseur){
					if(!partie.planete.atterrisseur){
						...
						for(var t=0; t < ListTagsObligatoires.atterrisseur.length; t++){
							objResult = Parties.aggregate([{$match: {_id: partie._id}}, {$unwind: "$deck.cartes"}, {$unwind: "$deck.cartes.tags"}, {$match: {"deck.cartes.tags": ListTagsObligatoires.atterrisseur[t].tag, "deck.cartes.active": true }}, {$group: {"_id":"$_id", "nbr":{$sum: 1}}}]);
							if(!objResult || objResult.length == 0 || objResult.nbr == 0){
								erreurs.isOK = false;
								erreurs.isValid = false;
								switch(niveauDetails){
									case 1:
										eCats[ListTagsObligatoires.atterrisseur[t].categorie] = 1;
										break;
									case 2: 
										erreurs.cartes.push({tag: ListTagsObligatoires.atterrisseur[t].tag, cartes: Cartes.find({tags: ListTagsObligatoires.atterrisseur[t].tag, cubesat: false},{_id: 1, carteId: 1, intitule: 1, categorie: 1}).fetch()});
										break;		
								}
							}
						}
...
getValeurDeRegle = function(_partieId, valeurStr){
	...
	/*
	db.parties.find({_id: 'mrTntnS6pk6Laweth', {$or:[{"deck.cartes.carteId": "O1"},{"deck.cartes.carteId": "O2"}]}});
	db.parties.find({_id: 'mrTntnS6pk6Laweth', $where: "this.deck.cartes.carteId == 'O1' || this.deck.cartes.carteId == 'O2'" });
