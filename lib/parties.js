...
if(Meteor.isServer){
	...
	Meteor.publish("getPartiesScenarios", function(){
		return Parties.find({},{"scenarioId":1});
	});
	Meteor.publish("getPartiesScores", function(){
		return Parties.find({},{"objectif":1, "cubesat":1, "planete":1, "score":1, "scenarioId":1, "tempsTotal":1, "dateModif": 1, "userId": 1});
	});
}
...
Meteor.methods({
	...
	*/
	checkScenarioUtilise: function(_scenarioId){
		...
		try{
			//console.log("scenarioId" + _scenarioId);
			//console.log("nbr parties" + Parties.find({scenarioId: _scenarioId}).count());
	...
	newPartie: function(_scenarioId){
		...
		var scenario;
  		...
  		try {
  			...
  			check(scenario, Match.ObjectIncluding({_id: String, active: Boolean }));
  		} catch(e){
  		...
  		var now = new Date();
		var partie = {userId: userId, nom: TAPi18n.__("partie.nouvelle"), dateDebut: new Date(), dateModif: new Date(), deck:{cartes:[], structures:{n:0, ids:[]}, energieMax:0, budgetMax:0, masseMax:0, volumeMax:0, energie:0, budget:0, masse:0, volume:0, science:0}, budgetLanceur: 1, eventIds:[], nbrEventsFromTimer: 0, lastEventTime: now.getTime(), scenarioId: scenario._id, score: 0, logs:[], peers:[], experts:[], chat:[], finie: false, active: true};	
  		try{
			partie.deck.masse     =  0;
			...
			} else if (scenario.initialisation.planetes.length == 1){
				partie.planete = _.find(Planetes, function(planete){ return planete.planeteId == scenario.initialisation.planetes[0]; });
			}
	...
	addCarteToDeck: function(partieId, scenario, carte, nStructure, dateModif){
		...
		try{
			...
			if(carte.categorie == "Z"){ //carte Structure
				...
				partie.deck.structures.energies.push(0);
			...
			if(affectedRow) {
				partie.deck.cartes.push({_carteId: carte._id, active: true,  carteId: carte.carteId, tags: carte.tags, categorie: carte.categorie, fiabilite: carte.fiabilite, nStructure: nStructure});
			}
	  					...
	  					if ((scenario.collaboration.tpsMaxParAppel == 0) || (partie.peers[c].dateDebut + scenario.collaboration.tpsMaxParAppel > now)) {
	  						if(partie.peers[c].dateDebut == 0){ //première réponse du pair à l'appel
								...
								if(scenario.collaboration.nbrMaxAppels > 0) {
  								...
  								if((scenario.collaboration.nbrAppelsGratuits <= nbrAppels) && (partie.deck.budget - scenario.collaboration.malusBudget  < 0)) {
								...
								if(affectedRow){
									if(scenario.collaboration.nbrAppelsGratuits <= nbrAppels) {
										partie.deck.budget = partie.deck.budget - scenario.collaboration.malusBudget;
										var evenements = [];
	  									var evenement;
										for(var e=0; e < scenario.collaboration.malusNbrEvenements; e++){ 
											evenement = Meteor.apply("addEvenementToDeck", [partie, Meteor.apply("getRandomEvenement", [partie.eventIds, false, scenario.initialisation.cubesat], {returnStubValue: true})], {returnStubValue: true}); 
											if((evenement != null) && (evenement.hasOwnProperty("eventId"))) evenements.push(evenement);
										}
										if(evenements.length > 0){
	  										if(Meteor.isClient) Modal.show("EvenementModal", {evenements: evenements});
	  									}
		...
		var tps = partie.tempsTotal + (now.getTime() - dateModif) / 1000;
  		try {
  			...
  			if((scenario.expertise.nbrExpertsGratuits > partie.experts.length) || (partie.deck.budget - scenario.expertise.malusBudget  >= 0)) {
  				...
  				//console.log("ligne 800");
  				if(scenario.expertise.nbrExpertsGratuits <= partie.experts.length) {
	  				var evenements = [];
	  				var evenement;
	  				//console.log("ligne 804");
					for(var e=0; e < scenario.expertise.malusNbrEvenements; e++){ 
						evenement = Meteor.apply("addEvenementToDeck", [partie, Meteor.apply("getRandomEvenement", [partie.eventIds, false, scenario.initialisation.cubesat], {returnStubValue: true})], {returnStubValue: true}); 
						if((evenement != null) && (evenement.hasOwnProperty("eventId"))) evenements.push(evenement);
						//console.log("ligne 808");
					}
					//console.log("ligne 810");
					if(evenements.length > 0){
						//console.log("ligne 812");
						if(Meteor.isClient)  Modal.show("EvenementModal", {evenements: evenements});
					}
					//console.log("ligne 815");
  				}
  			} else {
	...
	*/
			...
			if(affectedRow){
				...
				for(var i=0; i < evenement.targetCarteIds.length; i++){
					for(var c=0; c < partie.deck.cartes.length; c++){
						if(partie.deck.cartes[c]._carteId === evenement.targetCarteIds[i]) { partie.deck.cartes[c].active = false;  evenement.carteIds.push(partie.deck.cartes[c]._carteId); }
					}
				}
				for(var i=0; i < evenement.targetCarteTags.length; i++){
					for(var c=0; c < partie.deck.cartes.length; c++){
						if(contains(partie.deck.cartes[c].tags, evenement.targetCarteTags[i])) { partie.deck.cartes[c].active = false;  evenement.carteIds.push(partie.deck.cartes[c]._carteId); break; } //une seule carte désactivée par tag
					}
				}
				...
				if(evenement.isLancement){
					...
					Parties.update({_id: partie._id},{$set:{ "deck.cartes": partie.deck.cartes, "nbrEventsFromTimer": (partie.nbrEventsFromTimer+1), "lastEventTime": dateModif }});
				}	
	...
	launchEvenement: function(partieId){
		var partie;
  		try {
  			check(partieId, String);
  			partie = Parties.findOne({_id: partieId});
			...
			return throwAlert("error","bad-parameters", "Fonction launchEvenement : mauvais arguments");
		}	
		
  		if((partie.finie == 1) || (partie.active == 0)) { return  }
  		var scenario = Scenarios.findOne({_id: partie.scenarioId});
  		var now = new Date();
		//console.log(scenario);
  		//console.log(partie);
  		if(!partie.hasOwnProperty('nbrEventsFromTimer')) partie.nbrEventsFromTimer = 0;
  		if(!partie.hasOwnProperty('lastEventTime')) partie.lastEventTime = now.getTime();
  		if((scenario.evenement.intervalleTps > 0) && ((scenario.evenement.nbrMax == 0) || (scenario.evenement.nbrMax > partie.nbrEventsFromTimer ))) {
  			
  			//console.log(now);
  			if(Math.floor((now.getTime() - partie.lastEventTime) /1000) > scenario.evenement.intervalleTps){
  				/*partie.lastEventTime = now.getTime();
  				partie.nbrEventsFromTimer = partie.nbrEventsFromTimer +1;*/
  				//console.log("LANDCEMENT EVENT" );
  				var evenement = Meteor.apply("getRandomEvenement", [partie.eventIds, false, scenario.initialisation.cubesat], {returnStubValue: true});
  				//console.log(evenement);
  				evenement = Meteor.apply("addEvenementToDeck", [partie, scenario,  evenement, partie.lastEventTime], {returnStubValue: true});
  				if(Meteor.isClient)  Modal.show("EvenementModal", {evenements: [evenement]});
  			}
  		}
	},
	...
	lancementProjet: function(partieId, scenario, dateModif){
		...
		var erreurs = {cartes: [], evenementsFinaux: []};
  		...
  		if(user._id != partie.userId){ return throwAlert("error","privilege_user", TAPi18n.__("error.privilege_user")); }
  		//if((partie.finie == 1) || (partie.active == 0)) { return throwAlert("error","privilege_user", "PARTIE DEJA FINIE"); }
  		...
  		//console.log("ligne 897");
  		Meteor.call("updateConstantes",partie, scenario, dateModif);
  		//console.log("ligne 899");
		...
		//console.log("ligne 979");
  		...
  		if(scenario.lancement.nbrEvenements > 0){
  			...
  			for(var n=0; n < scenario.lancement.nbrEvenements; n++){
  				...
  				var affectedRow = false;
  				var now = new Date();
				evenement = Meteor.apply("addEvenementToDeck", [partie, scenario,  evenement, dateModif], {returnStubValue: true});
				if((evenement != null) && (evenement.hasOwnProperty("eventId"))) {
  			...
  			if(evenements.length > 0){
  				if(Meteor.isClient) {
  					Modal.show("EvenementModal", {evenements: evenements});
  					console.log("appel modal evenements");
  				} 
  			}
  			Meteor.call("updateConstantes",partie, scenario, dateModif);

  			rapportEvents = Meteor.apply("validationDeck", [partie, scenario, scenario.validation.niveauDetails, true], {returnStubValue: true});
  			rapportEvents.cartes = rapportEvents.cartes.concat(erreurs.cartes);
  			rapportEvents.evenementsFinaux = erreurs.evenementsFinaux;

  		}
		//console.log("ligne 1003");
  		try {
			affectedRow = Parties.update({_id: partie._id},{$set:{rapportFinal: rapport, rapportFinalEvents: rapportEvents, score: score, finie: true }});
			if(affectedRow){
				Meteor.call("throwNotification", "PROJECT-LAUNCHED", user._id, {scenarioId: scenario._id, partieId: partie._id, nom: partie.nom });
				//console.log("ligne 1008");
			}
		} catch(e){
			return throwAlert("error","database_request", TAPi18n.__("error.database_request", "lancementProjet"));
		  	console.error(TAPi18n.__("error.database_request", "lancementProjet") , e);
	...
	validationDeck: function(partie, scenario, niveauDetails, isLancement) {
		...
		var carte  = null;
		var valeur = 0;
		if(Meteor.isClient) return {cartes: []};
		  				...
		  				if(carte.regles.necessite.length > 0){
		  						...
		  						try {
			  						...
			  						if(!objResult || objResult.length == 0 || objResult.nbr == 0){
			  							console.log('erreur detectee' + carte.regles.necessite.replace(reg, carteDeck.nStructure));
		...
		if(partie.deck.energie < 0){
 		...
 		if(!partie.deck.structures.hasOwnProperty("masses")){
			partie.deck.structures.masses = [];
		}
	...
	scoreDeck: function(partie, scenario){
		...
		var score = partie.deck.science * scenario.score.ptsParScience;
		console.log("debug score : " + score);
		if(scenario.score.seuilTps < (partie.tempsTotal / 60)){
		 console.log("debug score : tmps:" + partie.tempsTotal + " seuilTps:" + scenario.score.seuilTps + "  ptsParTps:" + scenario.score.ptsParTps);
		 score = score - ((Math.floor(partie.tempsTotal / 60) - scenario.score.seuilTps) * scenario.score.ptsParTps);
		 console.log("debug score : " + score);
		}
		if(score < 0) return 0;
		return score;
	...
	*/
	updateConstantes: function(partie, scenario, dateModif){
		//if((partie.finie == 1) || (partie.active == 0)) { return throwAlert("error","partie_close", TAPi18n.__("error.partie_close")); }
		partie.deck.budget     = scenario.initialisation.budget - scenario.initialisation.budgetGestion;
		...
		if(!partie.deck.structures.hasOwnProperty("energies")){
			partie.deck.structures.energies = [];
		}
		//parcours des cartes du deck
		var carte  = null;
		var valeur = 0;
		//console.log("ligne 1416");
		...
		var nbrStructures = (_.filter(partie.deck.cartes, function(carte){ return (carte.categorie == "Z"); })).length;
		//console.log("nbr structure 1519 :" + nbrStructures);
		partie.deck.structures.masses = Array(nbrStructures).fill(0);
		partie.deck.structures.energies = Array(nbrStructures).fill(0);
		//console.log("ligne 1418 " + nbrStructures);
		//console.log("nbr carrtes " + partie.deck.cartes.length); 
		for(var c=0; c < partie.deck.cartes.length; c++){
			...
			//console.log("ligne 1429");

			//console.log(carte);
			if(!carte.active) { continue; }
			if(carte.cubesat != scenario.initialisation.cubesat) { continue; }
			//console.log("ligne 1432");
			if(partie.deck.cartes[c].active){//carte désactivée dans la partie, elle n'a plus d'impact sur l'énergie et la science mais bien sur le budget, la masse et le volume.
				...
				if(contains(carte.tags, "energieGenerateur")) {
					...
					if(partie.deck.cartes[c].nStructure > 0){
						partie.deck.structures.energies[partie.deck.structures.ids.indexOf(partie.deck.cartes[c].nStructure)] += valeur;
					}
				}  else {
					partie.deck.energie -= valeur;
					if(partie.deck.cartes[c].nStructure > 0){
			...
			if(contains(carte.tags, "lanceur")){//% du lanceur utilisé
				...
				//console.log("ligne 1467");
				if(partie.deck.cartes[c].nStructure > 0){
					//console.log("ligne 1469");
					if(partie.deck.cartes[c].categorie == "Z"){
						//console.log("ligne 1471");
						//console.log("PDS STRUCTURE");
						//console.log(partie.deck.structures.ids.indexOf(partie.deck.cartes[c].nStructure));
						//console.log(partie.deck.structures.masses);
						//console.log(partie.deck.cartes);
						//console.log("maxPds " + carte.maxPds);
						partie.deck.structures.masses[partie.deck.structures.ids.indexOf(partie.deck.cartes[c].nStructure)] += (carte.maxPds) - valeur;
					} else {
						//console.log("ligne 1478");
						partie.deck.structures.masses[partie.deck.structures.ids.indexOf(partie.deck.cartes[c].nStructure)] -= valeur;
					}
				}
				//console.log("ligne 1482");
				partie.deck.masse -= valeur;
			}	
			//console.log("ligne 1480");
		...
		var cZ4 = _.find(partie.deck.cartes, function(carte){ return (carte.carteId == "Z4"); });
		var cZ5 = _.find(partie.deck.cartes, function(carte){ return (carte.carteId == "Z5"); });
		if (cZ4 && cZ5) {
			partie.deck.structures.energies[partie.deck.structures.ids.indexOf(cZ4.nStructure)] = partie.deck.structures.energies[partie.deck.structures.ids.indexOf(cZ5.nStructure)];
			//partie.deck.structures.masses[partie.deck.structures.ids.indexOf(cZ4.nStructure)] = partie.deck.structures.masses[partie.deck.structures.ids.indexOf(cZ5.nStructure)];
		}
		//console.log("ligne 1485");
		//parcours des évènements
		
		var evenement = null;
		console.log(partie.eventIds);
		for(var e=0; e < partie.eventIds.length; e++){
			evenement = Evenements.findOne({_id: partie.eventIds[e]});
			console.log("update constante evenement"); console.log(evenement);
			console.log("science avant: " + partie.deck.science);
			if(!evenement.active) { continue; }
			if(evenement.cubesat != scenario.initialisation.cubesat) { continue; }
			
			partie.deck.energie = partie.deck.energie + evenement.deltaNrg;
			partie.deck.energieMax += ((evenement.deltaNrg > 0) ? evenement.deltaNrg : 0);
			...
			console.log("science après: " + partie.deck.science); 
		}
		
		//console.log("budget :" + partie.deck.budget);
