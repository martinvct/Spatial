...
Meteor.methods({
	...
	*/
	setScenarioToPartie: function(partie, scenario){
		...
		try{
			...
			affectedRow = Parties.update({_id: partie._id},{$set:{scenarioId: scenario._id, cubesat: scenario.initialisation.cubesat, "deck.budget": scenario.initialisation.budget, "deck.masse": partie.deck.masse, "deck.volume": partie.deck.volume}});
		} catch(e){
	...
	addCarteToDeck: function(partie, scenario, carte){
		...
		try{
			affectedRow = Parties.update({_id: partie._id}, 
				{$set: { dateModif: now },
			 	 $push: { "deck.cartes": {_carteId: carte._id, active: true,  carteId: carte.carteId, tags: carte.tags}, logs:  log}
				});
		} catch(e){
	...
	*/
	addEvenementToDeck: function(partie, scenario,  evenement){
		...
		var now = new Date();
		try {
			affectedRow = Parties.update({_id: partie._id},{$set:{ dateModif: now }, $push:{eventIds: evenement._id}});
			if(affectedRow){
				Meteor.call("updateConstantes",partie, scenario);
	...
	*/
	updateConstantes: function(partie, scenario){
		...
		for(var c=0; c < partie.deck.cartes.length; c++){
			...
			if(partie.deck.cartes[c].active){//carte désactivée dans la partie, elle n'a plus d'impact sur l'énergie et la science mais bien sur le budget, la masse et le volume.
				...
				if(contains(carte.tags, "lanceur")){//% du lanceur utilisé
					percent = partie.budgetLanceur;
				}
				if(typeof carte.valNrg === 'Number') {
					if(contains(carte.tags, "Energie")) {
						partie.deck.energie += percent * carte.valNrg;
					}  else {
						partie.deck.energie -= percent * carte.valNrg;
					}
				} else {
					try{
						...
						if(Parties.find(cond).count() == 1){
							if(contains(carte.tags, "Energie")) {
								partie.deck.energie += percent * carte.valNrg.valeur;
							}  else {
								partie.deck.energie -= percent * carte.valNrg.valeur;
							}
						} 
				...
				if(typeof carte.valSci === 'Number') {
					partie.deck.science += percent * carte.valSci;
				} else {
					try{
						var cond  = JSON.parse(carte.valSci.condition);
						cond._id  = partie._id
						if(Parties.find(cond).count() == 1) partie.deck.science += percent * carte.valSci.valeur;
					} catch(e) {
			...
			if(typeof carte.valEur === 'Number') {
				partie.deck.budget -= percent * carte.valEur;
			} else {
				try{
					var cond  = JSON.parse(carte.valEur.condition);
					cond._id  = partie._id
					if(Parties.find(cond).count() == 1) partie.deck.budget -= percent * carte.valEur.valeur;
				} catch(e) {
					console.error(TAPi18n.__("error.database_request", "addCarteToDeck_valEur") , e);
				}
			}
			if(typeof carte.valPds === 'Number') {
				partie.deck.masse -= percent * carte.valPds;
			} else {
				try{
					var cond  = JSON.parse(carte.valPds.condition);
					cond._id  = partie._id
					if(Parties.find(cond).count() == 1) partie.deck.masse -= percent * carte.valPds.valeur;
				} catch(e) {
					console.error(TAPi18n.__("error.database_request", "addCarteToDeck_valPds") , e);
				}
			}
			if(typeof carte.valVol === 'Number') {
				partie.deck.volume -= percent * carte.valVol;
			} else {
				try{
					var cond  = JSON.parse(carte.valVol.condition);
					cond._id  = partie._id
					if(Parties.find(cond).count() == 1) partie.deck.volume -= percent * carte.valVol.valeur;
				} catch(e) {
		...
		var affectedRow = false;
		try {
			affectedRow = Parties.update({_id: partie._id},{$set:{"deck.energie": partie.deck.energie, "deck.budget": partie.deck.budget, "deck.masse": partie.deck.masse, "deck.volume": partie.deck.volume, "deck.science": partie.deck.science}});
		} catch(e){
