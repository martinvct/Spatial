	...
	setScenario: function(scenario){
		...
		try{
			check(scenario, Match.ObjectIncluding({
				...
				description: String,
				active: Boolean,
				initialisation: { cubesat: Boolean, budget: Number, objectif: String, planetes: Match.Optional([String]), cubesatLimites: {poidsMax: Number, volumeMax: Number}  },
	    ...
	    if(result){
	  		...
	  		if(Meteor.isClient) sAlert.success(TAPi18n.__("message.scenario_enregistree", scenario.scenarioId + " " + scenario.intitule ));
	  		return scenario._id;
	  	}
	  	return 0;
	},
